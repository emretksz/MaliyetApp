...
public partial class ProductListPopup : Popup 
{
    ...
    public ProductListPopup(Sale sale = null, Order order=null)
	{
		InitializeComponent();
        ...
        try
        {
            ...
            if (saleList.Count > 0)
            {
                if (_order == null)
                {
                    ...
                    {
                        if (!await DatabaseService.GetOrderDetailCheckSaleWindows(order.Id, item.Id))
                        {
                ...
                else
                {
                   ...
                   await DatabaseService.UpdateOrder(_order);
                    ...
                    {
                        if (!await DatabaseService.GetOrderDetailCheckSaleWindows(_order.Id, item.Id))
                        {
                            OrderDetails detail = new OrderDetails()
                            {
                                OrderId = _order.Id,
